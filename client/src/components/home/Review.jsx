...
const ReviewCard = memo(({item: { review, rate, name, img }}) => {
  return (
    <div className='h-full w-full bg-white pt-8 pb-5 pl-8 pr-5 rounded-3xl shadow-sm group'>
    ...
    <section className='h-full w-full py-8 md:py-16 bg-bg-brown'>
      <div className='sm:container mx-auto px-4'>
        <div className='pb-16 flex flex-col justify-center items-center'>
          ...
          <Heading text={"Results That Stand Strong"} />
        </div>
        ...
        :
          (<Suspense fallback={<div className="p-24 text-center">Loading...</div>}>
            <EmblaSlider
              items={review}
              renderSlide={(item) =><ReviewCard key={item.name} item={item} />}
              autoplay
              autoplayDelay={3000}
              loop = {true}
              arrows={true}
